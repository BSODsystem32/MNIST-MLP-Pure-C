...
static void free_test(TestData *d) {
    free(d->images);
    free(d->labels);
    d->images = NULL;
    d->labels = NULL;
    d->count = d->rows = d->cols = 0;
}
...
static int load_test(const char *img_path, const char *lbl_path, TestData *d) {
    ...
    d->count = (int)read_be32(fi);
    (void)read_be32(fl);
    d->rows  = (int)read_be32(fi);
    d->cols  = (int)read_be32(fi);

    int px = d->rows * d->cols;
    size_t image_bytes = (size_t)d->count * (size_t)px;

    d->images = (uint8_t *)malloc(image_bytes);
    d->labels = (uint8_t *)malloc((size_t)d->count);
    if (!d->images || !d->labels) {
        fprintf(stderr, "OOM\n");
        free_test(d);
        fclose(fi);
        fclose(fl);
    ...
    if (fread(d->images, 1, image_bytes, fi) != image_bytes ||
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < d.count; i++) {
        int pred = infer_forward_u8(net, &scratch, d.images + (size_t)i * pixels);
    ...
    for (int w = 0; w < 3; w++)
        for (int i = 0; i < d.count; i++)
            infer_forward_u8(net, &scratch, d.images + (size_t)i * pixels);

    const int BENCH_PASSES = 10;
    clock_t t0 = clock();
